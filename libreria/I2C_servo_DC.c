/*Basado en librería de Ligo George, Electrosome
 * File:   I2C_SDC.c
 * Author: Carlos
 *
 * Created on 22 de febrero de 2020, 11:33 PM
 */
...
void I2C_Master_Init(const unsigned long c)         //Inicia configuracion para I2C Master
{
    SSPCON = 0b00101000;                    //I2C clk=F0sc/(4(SSPADD+1))
    SSPCON2 = 0;
    SSPADD = (_XTAL_FREQ/(4*c))-1;          //Carga el valor 9 para frec 100kHz
    SSPSTAT = 0;
    TRISCbits.TRISC3 = 1;
    TRISCbits.TRISC4 = 1;
}
...
{
    while ((SSPSTAT & 0x04) || (SSPCON2 & 0x1F));       //Si es master, hay transmision. si es slave, espera a tener control dl SCL
}
//*****************************************************************************
